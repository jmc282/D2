...
class Game < Location
  ...
  ANGELS_CAMP = Location.new "Angel's Camp", 0, 4
  NEVADA_CITY = Location.new 'Nevada City', 0, 5
  VIRGINIA_CITY = Location.new 'Virginia City', 3, 3
  MIDAS = Location.new 'Midas', 5, 0
  EL_DORADO = Location.new 'El Dorado Canyon', 10, 0
  ...
  def display_location_message
    puts "Prospector #{@which_player} #{PLAYER.current_location.name}."
  end
  
  # Finds which location the miner heads to next given the current location
  def next_location(location) 
    puts location.amt_neighbors
    num = random_int(0, location.amt_neighbors) 
    puts num
